...
import "./status/aircity/ac_api.min";
import { AirCityAPI, AirCityPlayer } from "./status/aircity/ac.min";
import HostConfig from "./status/aircity/ac_conf";
...
const App: FC = () => {
  ...
  useEffect(() => {
    const initInterface = (withPlayer: boolean, withApi: boolean) => {
      ...
      //AirCityAPI
      if (withApi) {
        let a = new AirCityAPI(HostConfig.instanceId || HostConfig.AirCityAPI, {
          // onReady: () => {
          //   //此时可以调用接口了
          //   if (window.__g) {
    ...
    const init = (withPlayer: boolean, withInterface: boolean) => {
      if (window.location.search.indexOf("ms") !== -1) {
        ...
        getMatchServerConfig(
          HostConfig.MatchServer,
          function (o: { result: number; instanceId: string }) {
            if (o.result === 0) {
